...
// Check if IC and ID are provided in the URL
if (isset($_GET['ic']) && isset($_GET['id'])) {
    // Retrieve IC and ID from the URL
    $ic = $_GET['ic'];
    $id = $_GET['id'];
    ob_clean();
    ...
    $query_form1 = "SELECT * FROM form1 WHERE ic = '$ic'";
    $result_form1 = mysqli_query($conn, $query_form1);

    if (mysqli_num_rows($result_form1) > 0) {
        ...
        // Handle case where no data is found for form1
        $pdf->Cell(0, 10, 'No data found for form1', 0, 1);
    }

    // Fetch data from form2
    $query_form2 = "SELECT * FROM form2 WHERE ic = '$ic'";
    $result_form2 = mysqli_query($conn, $query_form2);

    if (mysqli_num_rows($result_form2) > 0) {
        $row_form2 = mysqli_fetch_assoc($result_form2);
        $pdf->Cell(0, 10, 'Perkhidmatan: ' . $row_form2['perkhidmatan'], 0, 1);
    } else {
        // Handle case where no data is found for form2
        $pdf->Cell(0, 10, 'No data found for form2', 0, 1);
    }

    // Fetch data from form2b
    $query_form2b = "SELECT * FROM form2b WHERE ic = '$ic' AND id = '$id'";
    $result_form2b = mysqli_query($conn, $query_form2b);

    if (mysqli_num_rows($result_form2b) > 0) {
        ...
        // Handle case where no data is found for form2b
        $pdf->Cell(0, 10, 'No data found for form2b', 0, 1);
    }

    // Fetch data from form3a
    $query_form3a = "SELECT * FROM form3b WHERE ic = '$ic' AND id = '$id'";
    $result_form3a = mysqli_query($conn, $query_form3a);

    if (mysqli_num_rows($result_form3a) > 0) {
        ...
        // Handle case where no data is found for form3a
        $pdf->Cell(0, 10, 'No data found for form3a', 0, 1);
    }

    // Fetch data from form4a
    $query_form4a = "SELECT * FROM form4b WHERE ic = '$ic' AND id = '$id'";
    $result_form4a = mysqli_query($conn, $query_form4a);

    if (mysqli_num_rows($result_form4a) > 0) {
        ...
        // Handle case where no data is found for form4a
        $pdf->Cell(0, 10, 'No data found for form4a', 0, 1);
    }
