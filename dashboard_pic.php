...
WHERE form2.perkhidmatan = 'audio_visual' and form2a.pic = '".$_SESSION['name']."'";
//echo $sql2;
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "</td>";
		 echo "<td>";
echo "<button onclick='approveRequest(\"" . $row["ic"] . "\", \"audio_visual\")'>Approve</button>";
echo "<button onclick='rejectRequest(\"" . $row["ic"] . "\", \"audio_visual\")'>Reject</button>";
echo "<button onclick='completeRequest(\"" . $row["ic"] . "\", \"audio_visual\")'>Complete</button>";
...
LEFT JOIN user ON form2.ic = user.ic 
WHERE form2.perkhidmatan = 'kerja_grafik_multimedia' and form2b.pic = '".$_SESSION['name']."'";
$result = $conn->query($sql2);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        echo "<tr>";
        ...
        echo "</td>";
				 echo "<td>";
echo "<button onclick='approveRequest(\"" . $row["ic"] . "\", \"kerja_grafik_multimedia\")'>Approve</button>";
echo "<button onclick='rejectRequest(\"" . $row["ic"] . "\", \"kerja_grafik_multimedia\")'>Reject</button>";
echo "<button onclick='completeRequest(\"" . $row["ic"] . "\", \"kerja_grafik_multimedia\")'>Complete</button>";
...
function updatePIC1(ic, button) {
	...
	function approveRequest(ic, perkhidmatan) {
...
function rejectRequest(ic, perkhidmatan) {
