...
if(isset($_POST['id'])) {
    ...
    $update_query = "UPDATE form2a SET approval_status = 2 WHERE id = '$id'";

    if(mysqli_query($conn, $update_query)) {
        // Retrieve email of the user associated with the request
        $email_query = "SELECT * FROM form2a WHERE id = '$id'";
        ...
        if($email_result && mysqli_num_rows($email_result) > 0) {
            ...
            $mail->setFrom('dineshegas@outlook.com', 'Audio Visual Rejection Notification');
            $mail->addAddress($email);

            $mail->isHTML(true);
            $mail->Subject = "Request Rejection Notification";
            $mail->Body = "<p><p>Dear user (IC: $ic), your Audio Visual request has been rejected.</p>";
